                            ...
                            <table class="min-w-full bg-white border border-gray-200">
                                <thead>
                                    <tr class="bg-gray-100">
                                        ...
                                        <th class="py-2 px-4 border-b text-left">Bank Account Number</th>
                                ...
                                <tbody>
                                    @foreach ($pendingUsers as $user)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="py-2 px-4 border-b">
                                                {{ $user->organizerProfile->bank_name ?? '-' }}
                                            </td>
                                            <td class="py-2 px-4 border-b">
                                                {{ $user->organizerProfile->bank_account_number ?? '-' }}
                                            </td>

                                            <td class="py-2 px-4 border-b">
                                                {{-- 1. Check if profile exists AND if document_path exists --}}
                                                @if ($user->organizerProfile && $user->organizerProfile->document_path)
                                                    <a href="{{ asset('storage/' . $user->organizerProfile->document_path) }}"
                                                        target="_blank"
                                            ...
                                            <td class="py-2 px-4 border-b text-center">
                                                <form action="{{ route('admin.approve', $user->id) }}" method="POST"
                                                    onsubmit="return confirm('Yakin verifikasi user ini?');">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit"
